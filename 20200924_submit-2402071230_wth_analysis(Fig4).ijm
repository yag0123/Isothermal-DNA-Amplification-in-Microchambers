...
function main() {
	...
	for (fieldIdx=START_FIELD_IDX; fieldIdx<=END_FIELD_IDX; fieldIdx++) {
		...
		if (LOAD_ROI) {
			...
			fovRoiPresence = detectRoiParameters[12];
		}
...
function registerRoi(imgTitle, channelIdx, timepoint, detectRoiParameters, fovnumber) {	
	...
	fovRoiPresence = detectRoiParameters_new[12];
	thisFovRoiPresence = substring(fovRoiPresence, lengthOf(fovRoiPresence)-1, lengthOf(fovRoiPresence));
...
function setRoiSettingArea(imgTitle, detectRoiParameters) {
	...
	screenregion_use_erosion = detectRoiParameters[13];
	screenregion_erosion_radius = detectRoiParameters[14];
	screenregion_roich_lower = detectRoiParameters[15];
	screenregion_roich_upper = detectRoiParameters[16];
	skipdialog_screenregion = detectRoiParameters[17];
	...
	detectRoiParameters_new[13] = screenregion_use_erosion;
	detectRoiParameters_new[14] = screenregion_erosion_radius;
	detectRoiParameters_new[15] = screenregion_roich_lower;
	detectRoiParameters_new[16] = screenregion_roich_upper;
	detectRoiParameters_new[17] = skipdialog_screenregion;
	return detectRoiParameters_new;
}
...
//detectRoiParameters: prom, maximaok, roichblur, roicenterstabletitle, roiRadius 230309
	...
	fovRoiPresence = detectRoiParameters[12];
	skipdialog_findmaxima = detectRoiParameters[18];
	//230309
	...
	detectRoiParameters_new[12] = fovRoiPresence; //240201
	detectRoiParameters_new[18] = skipdialog_findmaxima; //240201
...
function setRoi(roiCentersTableTitle, detectRoiParameters, fovnumber) {
	...
	skipdialog_roiradius = detectRoiParameters[19];
	detectRoiParameters_new = detectRoiParameters;
	...
	detectRoiParameters_new[19] = skipdialog_roiradius;
...
function exportRogPreview(imgTitle, channelIdx, workDir, resultTableTitle, imgIdentifier, exportRogPrevParameters){
	...
	if(slices > 1 || frames > 1){
		...
		if(skipdialog_exportrog == false){
			Dialog.create("Make ROG separation preview image");
...
function saveLog(path, filename){
	...
	print(f, "fov_presence_roi" + "\t" + detectRoiParameters[12] + "\n");
	print(f, "screenregion_use_erosion" + "\t" + detectRoiParameters[13] + "\n"); 
	print(f, "screenregion_erosion_radius" + "\t" + detectRoiParameters[14] + "\n"); 
	print(f, "screenregion_roich_lower" + "\t" + detectRoiParameters[15] + "\n"); 
	print(f, "screenregion_roich_upper" + "\t" + detectRoiParameters[16] + "\n"); 
	if(EXPORT_ROG){
		if(DUALCOLOR){
...
#@ Boolean (label="★ROG値の書き出しも行う", value=false, discription="輝点ごとのROG値（8x8切り出し）をcsv形式で書き出す") EXPORT_ROG
...
//detectRoiParameters: prom, maximaok, roichblur, roicenterstabletitle, roiRadius, findbeads-channelBFnum, findbeads-minarea, findbeads-maxarea, findbeads-tolerance, findbeads-doinvertedanalysis,
// (continued) findbeads-BTHradius, fov_roiPresence, screenregion_use_erosion, screenregion_erosion_radius, screenregion_roich_lower, screenregion_roich_upper, skipdialog_screenregion, skipdialog_findmaxima, skipdialog_roiradius 240201
detectRoiParameters_default = newArray(100, false, "No", "Untitled", -1,  1, 2, 100, 0, 0,  6, 3, "", true, 10, 0,  2000, false, false, false);
//exportRogPrevParameters  thresRogUL, thresIntLL, saveRogRoi, 2ndch-thresRogUL, 2ndch-thresIntLL, 2ndch-saveRogRoi, skipdialog_exportrog, intRangeMin, intRangeMax 230310 
